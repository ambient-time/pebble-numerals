...
Open Numerals settings in the Pebble phone app. Choose a numeral system and either **Hours and minutes** or **Hours, minutes, seconds**, then select **Save to watch**. Each choice has a reading guide. Hours use local 24-hour time. Most designs read down the screen; Maya, Cistercian and Ogham read across it. Some systems use modern zero conventions or fictional signs; the guide explains these adaptations. Braille is visual, on a flat screen.
...
Numerals 0.1.0 targets Basalt, Diorite, Emery and Flint. The frozen PBW and portable source ZIP are in `release/`; `evidence/index.html` lets you inspect every choice on every target in both modes. Native emulator checks are separate from physical-watch readability, battery life and real-phone delivery, which remain unverified. See `release/publication.json` in the main project for the current store receipt.
