...
pbw=release/f'numerals-{version}.pbw';shutil.copyfile(ROOT/'build'/f'{ROOT.name}.pbw',pbw)
source=release/f'numerals-{version}-source.zip'
files=[p for p in ROOT.rglob('*') if p.is_file() and not any(x in ['.git','build','release','evidence','node_modules','__pycache__'] for x in p.relative_to(ROOT).parts) and not p.name.startswith('.lock-waf') and p.suffix!='.pyc']
